...
import { getTrades, deleteTrade, updateTrade, getStrategies } from '@/lib/storage';
...
export default function TradeLibrary() {
  ...
  // FIX 12: Dynamic Page Title
  useEffect(() => {
    const originalTitle = document.title;
    if (isModalOpen && selectedTrade) {
      document.title = `${selectedTrade.instrument} ${selectedTrade.direction} — SMC Journal`;
    }
    return () => {
  ...
  const handleUpdate = async (formData) => {
    ...
    try {
      ...
      setIsEditMode(false);
      showToast('Trade refined successfully.', 'success');
    } catch (err) {
      console.error('Update error:', err);
      showToast('Failed to refine execution.', 'error');
    } finally {
